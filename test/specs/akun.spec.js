...
//export const AkunTests = () => {
    describe('Akun Page Testing', () => {
        it('should navigate to Akun', async () => {
            ...
            await Navbar.akunButton.waitForDisplayed({
                timeout: 5000,
                timeoutMsg: 'Menu Akun di Bottom Navbar tidak muncul dalam batas waktu yang ditentukan'
            });
        ...
        it('should verifikasi icon copy inapass id', async () => {
            ...
            const isCopySuccessVisible = await AkunPage.copySuccess.waitForDisplayed({
                timeout: 10000,
                timeoutMsg: 'Pop Up INA Pas ID berhasil disalin tidak tampil dalam waktu diharapkan'
            });
        ...
        it('should view certificate detail', async () => {
            ...
            const isCekSertifikatVisible = await AkunPage.popupSertifikat.waitForDisplayed({
                timeout: 10000,
                timeoutMsg: 'Pop Up Sertifikat tidak muncul setelah mengklik menu Cek Sertifikat'
            });
        ...
        it('should navigation to PIN Baru screen with valid PIN', async () => {
            ...
            const isPinBaruVisible = await AktifkanPINPage.pinBaru.isDisplayed().catch(() => false);
        ...
        it('should not allow navigation to screen Masukkan PIN baru Kembali with PIN less than 6 digits', async () => {
            ...
            const isKonfirmasiPINVisible = await AktifkanPINPage.verifPinBaru.isDisplayed();
        ...
        it('should navigation to verifikasi New PIN screen with PIN 6 digits', async () => {
            ...
            // Langkah 2: Verifikasi bahwa screen konfirmasi PIN tidak muncul
            const isVerifikasiPINBaruVisible = await AktifkanPINPage.verifPinBaru.isDisplayed();
        ...
        it('should navigation to Biometric screen with PIN 6 digits Match', async () => {
            ...
            // Langkah 2: Verifikasi bahwa screen konfirmasi PIN tidak muncul
            const isPINBerhasilDiubahVisible = await await browser.waitUntil(async () => {
                return await AktifkanPINPage.pinBerhasilDiubah.isDisplayed();
            }, {
                timeout: 20000,  // Timeout maksimal, misalnya 20 detik
                timeoutMsg: 'Elemen PIN Diubah Berhasil tidak muncul dalam batas waktu yang ditentukan'
            });
    ...
    if (!isAkunScreenSelected) {
        ...
        expect(isAkunScreenSelected).toBe(true);
           ...
           await $('android=new UiScrollable(new UiSelector().scrollable(true)).scrollIntoView(new UiSelector().description("Keluar dari perangkat"))');
           
           await AkunPage.kebijakanPrivasi.click();
           await AkunPage.handleBrowserSelection();

           const iskebijakanPrivasiScreenVisible = await AkunPage.kebijakanPrivasiScreen.waitForDisplayed({
                timeout: 10000,
                timeoutMsg: 'Screen Akun tidak muncul setelah mengklik menu Akun'
            });
        ...
        it('should navigation to Transaksi', async () => {
            ...
            await AkunPage.riwayatTransaksi.waitForDisplayed({
                timeout: 10000,
                timeoutMsg: 'Screen Transaksi tidak muncul setelah mengklik menu Transaksi'
            });
        ...
        it('should navigation to Kartu', async () => {
            ...
            await Navbar.kartuButton.waitForDisplayed({
                timeout: 5000,
                timeoutMsg: 'Menu Kartu di Bottom Navbar tidak muncul dalam batas waktu yang ditentukan'
            });
...
if (!isAkunScreenSelected) {
    ...
    expect(isAkunScreenSelected).toBe(true);
       ...
       await $('android=new UiScrollable(new UiSelector().scrollable(true)).scrollIntoView(new UiSelector().description("Keluar dari perangkat"))');
           ...
           const isMessageKeluarInstruction = await AkunPage.messageKeluarPerangkat.waitForDisplayed({
                timeout: 5000,
                timeoutMsg: 'Pesan Instruction Keluar tidak muncul dalam batas waktu yang ditentukan'
            });
        ...
        it('should pop up to exit', async () => {
           ...
           const isPopUpKeluar = await AkunPage.popupKeluar.waitForDisplayed({
                timeout: 5000,
                timeoutMsg: 'Pop Up Keluar Keluar tidak muncul dalam batas waktu yang ditentukan'
            });
